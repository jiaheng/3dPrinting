...
Spring.make = function(specification, params) {
	...
	cylinder = cylinder.translate([0,0,(cylinderSpec.height-specification.thickness)/2]);
	...
	cylinder = Circle.make(cylinderSpec, params);
	cylinder = cylinder.translate([specification.roundedCubeLength/2,0,0]);
	connect = cube.union(cylinder);
	connect = connect.rotateZ(-90).translate(
				[0,
				-specification.innerCylinderRadius - specification.roundedCubeLength/2 + 1,
	...
	* No longer needed
	...
	cube = Rectangle.make(cubeSpec, params);
	cube = cube.rotateZ(90);
	cube = cube.translate([0,
			-specification.roundedCubeLength/2,
			specification.height - (specification.roundedCubeHeight+specification.thickness)/2]);

	spring = spring.union(cube);
	// rounded cube on one side of the rectangle
	cylinder = CSG.cylinder({
				start: [0,
					-specification.roundedCubeLength,
					specification.innerCylinderHeight - specification.thickness/2],
				end: [0,
					-specification.roundedCubeLength,
					specification.innerCylinderHeight - specification.thickness/2 - specification.roundedCubeHeight],
				radius: specification.roundedCubeWidth/2,
...
/*
 * No longer needed
function removeCentreHole(spring, specification, params) {
  if(specification.centreHoleRadius > 0) {
    ...
    spring = spring.subtract(centerHole);
  } 
  return spring;
};
*/

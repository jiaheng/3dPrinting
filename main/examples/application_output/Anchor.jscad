...
Anchor.make = function(specification, params) {
  ...
  var connectorWidth = specification.handWidth + 2; // connector must be wider
  cubeSpec.width = connectorWidth;
  cubeSpec.length = connectorLength;
  connector = Rectangle.make(cubeSpec, params);
  connector = connector.translate([(specification.maxRadius+connectorLength)/2,0,0]);
  ...
  hole = hole.translate([specification.maxRadius/2+connectorLength-holeLength/2,0,0]);
